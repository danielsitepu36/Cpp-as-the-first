...
int binarysearch(siswa data[],int j,long s);
...
int main(){
    ...
    n=15;
    selection(n,data,x);
    cout<<endl<<"Hasil setelah sorting menggunakan insertion sort : "<<endl;
    showtable(data);
    cout<<"Jumlah perpindahan posisi : "<<x<<endl<<endl;
    cout<<"Masukkan nilai rerata lama online yang ingin anda cari = ";
    cin>>s;
    f=binarysearch(data,n,s);
    if(f==-1){
        cout<<"Tidak ada"<<endl;
    }
    else{
        cout<<"Ditemukan !"<<endl;
        cout<<"NIM             = "<<data[f].NIM<<endl;
    }
    system("PAUSE");
}
...
int binarysearch(siswa data[],int j,long s){
    ...
    while(!found && i<=j){
        k=(i+j)/2;
        if(data[k].LamaOnline<s){
            i=k+1;
        }
        else if(data[k].LamaOnline==s){
            found=true;
        }
        else{
            j=k-1;
