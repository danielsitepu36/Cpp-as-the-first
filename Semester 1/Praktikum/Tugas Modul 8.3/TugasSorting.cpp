...
void binarysearch(tabel data[],long long unsigned query,int j);
...
void binarysearch(tabel data[],long long unsigned s,int j){
    bool found=false;
    int i=0,k;
    while(!found && i<=j){
        k=(i+j)/2;
    ...
    if(!found){
        ...
        cout<<"Ditemukan !"<<endl;
        cout<<"NISN = "<<data[k].NISN<<endl;
        cout<<"Nama = "<<data[k].nama<<endl;
        cout<<"Nilai = "<<data[k].nilai<<endl;
    }
}
